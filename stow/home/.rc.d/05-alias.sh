# shellcheck shell=sh
[ "${DOTFILE_DEBUG:-}" ] && echo "$(date '+%Y-%m-%d %H:%M') :: :: ALIASES_LOADED" | tee -a /tmp/shell-init.txt
...
alias h="history"
# alias j="jobs"    # now used for journal function
...
# export COMPOSE_FILE=~/docker/$(uname -n)/docker-compose.yml
