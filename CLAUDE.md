...
Dotfiles are managed with [GNU Stow](https://www.gnu.org/software/stow/) from the `stow/` directory, targeting `~`. The bootstrap script dispatches stow packages based on OS, hostname, and shell.

## Commands

### Setup (first time)
```bash
make deps        # Install bats, shellcheck, shfmt via mise or brew
